...
def hephaestus_projection_panels():
    class DREAM_PT_heph_panel_projection(bpy.types.Panel):
        """Creates a Hephaestus Panel for displacement projection"""
        bl_label = "Dream Texture"
        bl_idname = "DREAM_PT_heph_panel"
        bl_category = "Heph"
        bl_space_type = 'VIEW_3D'
        bl_region_type = 'UI'
    ...
    DREAM_PT_heph_panel_projection.__name__ = "DREAM_PT_heph_panel"
    yield DREAM_PT_heph_panel_projection

    def get_prompt(context):
            return context.scene.dream_textures_prompt
    
    yield from create_panel('VIEW_3D', 'UI', DREAM_PT_heph_panel_projection.bl_idname, prompt_panel, get_prompt)
    #yield create_panel('VIEW_3D', 'UI', DREAM_PT_heph_panel_projection.bl_idname, size_panel, get_prompt)
    #yield from create_panel('VIEW_3D', 'UI', DREAM_PT_heph_panel_projection.bl_idname, advanced_panel, get_prompt)
    def actions_panel(sub_panel, space_type, get_prompt):
        class ActionsPanel(sub_panel):
            """Create a subpanel for actions"""
            bl_idname = f"DREAM_PT_dream_panel_projection_actions"
            ...
            def draw(self, context):
                ...
                try:
                    ...
                    print(e)
        return ActionsPanel
    yield create_panel('VIEW_3D', 'UI', DREAM_PT_heph_panel_projection.bl_idname, actions_panel, get_prompt)
